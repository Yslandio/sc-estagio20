...
{ // Retorna todos os campos da tabela de feriados
    ...
    $feriados = $connection->query("SELECT * FROM feriados WHERE holidayDate LIKE '%$ano%' ORDER BY holidayDate ASC");
...
{ // Retorna as data de feriado / não letiva do banco de dados com base em um ano ou tudo caso não especifique o ano
    ...
    $feriados = $connection->query("SELECT holidayDate FROM feriados WHERE holidayDate LIKE '%$ano%' ORDER BY holidayDate ASC");
